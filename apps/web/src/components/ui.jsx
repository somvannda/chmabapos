...
function Field({ label, hint, type = "text", ...props }) {  const isPassword = type === "password";  const [showPassword, setShowPassword] = useState(false);  const input = <input type={isPassword && showPassword ? "text" : type} className={`h-11 w-full rounded-xl border border-[#dfdfe8] bg-white px-3.5 text-sm text-[#22232a] outline-none transition placeholder:text-[#aaabb4] focus:border-[#887bf3] focus:ring-4 focus:ring-[#6957f5]/10${isPassword ? " pr-10" : ""}`} {...props} />;  return (    <label className="block">      <span className="mb-1.5 block text-xs font-semibold text-[#4f5059]">{label}</span>      {isPassword ? <div className="relative">{input}<button type="button" aria-label={showPassword ? "Hide password" : "Show password"} onClick={() => setShowPassword((visible) => !visible)} className="absolute right-2.5 top-2.5 flex h-6 w-6 items-center justify-center rounded-md text-[#b0b1ba] transition hover:bg-[#f2f2f6] hover:text-[#6957f5]">{showPassword ? <EyeOff size={15} /> : <Eye size={15} />}</button></div> : input}      {hint && <span className="mt-1.5 block text-[11px] text-[#92939d]">{hint}</span>}    </label>  );}
...
function Logo({ light = false }) {  const chip = light ? "bg-[#c4f27c]" : "bg-[#17181c] dark:bg-[#c4f27c]";  const ring = light ? "border-[#17181c]" : "border-white dark:border-[#17181c]";  const word = light ? "text-white" : "text-[#17181c] dark:text-white";  return (    <div className="flex items-center gap-2.5">      <div className={`relative flex h-8 w-8 items-center justify-center overflow-hidden rounded-[10px] ${chip}`}>        <span className={`absolute h-3 w-3 rounded-full border-[2.5px] ${ring}`} />        <span className={`absolute -right-0.5 top-1.5 h-3 w-3 rounded-full border-[2.5px] ${ring}`} />      </div>      <span className={`text-[19px] font-extrabold tracking-[-.04em] ${word}`}>chmaba</span>    </div>  );}
